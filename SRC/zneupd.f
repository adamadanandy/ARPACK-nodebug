...
c         call zvout(logfil, ncv, workl(irz), ndigit,
c     &   '_neupd: Ritz values passed in from _NAUPD.')
c         call zvout(logfil, ncv, workl(ibd), ndigit,
...
c            call zvout (logfil, ncv, workl(bounds), ndigit,
...
c            call zvout (logfil, ncv, workl(iheig), ndigit,
c     &           '_neupd: Eigenvalues of H')
c            call zvout (logfil, ncv, workl(ihbds), ndigit,
c     &           '_neupd: Last row of the Schur vector matrix')
c            if (msglvl .gt. 3) then
c               call zmout (logfil       , ncv, ncv   , 
...
c
c            if (msglvl .gt. 2) then
c               call zcopy(nconv, workl(invsub+ncv-1), ldq,
c     &                    workl(ihbds), 1)
c               call zvout (logfil, nconv, workl(ihbds), ndigit,
...
c         call zvout (logfil, nconv, d, ndigit,
